...
//     }
    void addToLedger(string fromPk, string toPk, double amount, string sign, string prevHash, string newHash, int transactionId);
//     void addToLedger(string fromPk, string toPk, double amount, string sign, string prevHash, string newHash, int transactionId) {
//         standardLedger.addTransaction(prevHash, transactionId, fromPk, toPk, amount, sign, newHash);
//     }
...
//     }
    ...
    static string generateHash(string prevHash, int txnId, string fromPk, string toPk, double amount, string sign);
//     static string generateHash(string prevHash, int txnId, string fromPk, string toPk, double amount, string sign) {
//         string input = prevHash + to_string(txnId) + fromPk + toPk + to_string(amount) + sign;
//         hash<string> hasher;
//         size_t hash = 0;
//         for (char c : input) {
...
//     }
    ...
    static bool verifySignature(const string& publicKey, const string& signature, const string& data);
//     static bool verifySignature(const string& publicKey, const string& signature, const string& data) {

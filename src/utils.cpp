...
void deleteNodeAndRevertBalance(string recipientPublicKey,
                                string senderPublicKey, double amount,
                                Wallet wallets[]) {
...
Ledger send(string recipientPublicKey, string senderPublicKey, double amount,
            Wallet wallets[]) {  // is it a list of wallets?
    ...
    for (int i = 0; i < Wallet::walletCounter; i++) {
        if (&wallets[i] != senderWallet) {
            wallets[i].addToLedger(senderPublicKey, recipientPublicKey, amount,
                                   transacSignature);
        }
    }
    ...
    for (int i = 0; i < Wallet::walletCounter; i++) {
        map<string, string> walletDetials =
            wallets[i].getLastTransactionGraph();
    ...
    // If there is more than one hash with the maximum frequency, delete the
    // node and revert the balance
    if (maximumFrequencyHashes.size() > 1) {
        deleteNodeAndRevertBalance(recipientPublicKey, senderPublicKey, amount,
                                   wallets);
        return Ledger();
    }

    string maximumFrequencyHash = maximumFrequencyHashes[0];
    // TODO: Handle the case where multiple chains have the same maximum
    // frequency

    vector<Wallet> failedWallets;

    for (int i = 0; i < Wallet::walletCounter; i++) {
        if (wallets[i].getLastTransactionGraph()["Final Hash"] !=
            maximumFrequencyHash) {
            cout << "Chain verification failed for Wallet " << i + 1
                 << ". Taking maximum frequency chain." << endl;
    ...
    DLS dls;
    map<string, string> lastTransactionDetails =
        successfulWallet->getLastTransactionGraph();
    dls.standardLedger.addTransaction(
